    ...
    <div className='grid grid-cols-1 lg:grid-cols-2 gap-6'>
      <Card className="flex flex-col">
        ...
        <CardContent className="flex-1 pb-0" >
          <ChartContainer config={paymentChartConfig}  className="mx-auto aspect-square max-h-[250px]">
            <PieChart>
              ...
              <Pie data={paymentData} dataKey={"users"} nameKey={"method"}
              innerRadius={60 } strokeWidth={5}>
                <Label content={({viewBox})=>{
                  if(viewBox && "cx" in viewBox && "cy" in viewBox){
                    return <text x={viewBox.cx} y={viewBox.cy} textAnchor='middle' dominantBaseline={"middle"}>
                      <tspan x={viewBox.cx} y={viewBox.cy} className='fill-muted-foreground text-xl font-bold'>
                        {cod + online} Users
                      </tspan>
                     {}
                    </text>
        ...
        <CardFooter className="flex-col gap-2 text-sm ">
          <div className="leading-none text-muted-foreground">Showing total users for payment metods</div>
        </CardFooter>
      </Card>
      ...
      <Card className="flex flex-col">
        ...
        <CardContent className="flex-1 pb-0" >
          <ChartContainer config={paymentChartConfig}  className="mx-auto aspect-square max-h-[250px]">
            <PieChart>
              <ChartTooltip cursor={false} content={<ChartTooltipContent hideLabel/>}/>
              <Pie data={paymentPercentage} dataKey={"percentage"} nameKey={"method"}
              innerRadius={60 } strokeWidth={5}>
                <Label content={({viewBox})=>{
                  if(viewBox && "cx" in viewBox && "cy" in viewBox){
                    return <text x={viewBox.cx} y={viewBox.cy} textAnchor='middle' dominantBaseline={"middle"}>
        ...
        <CardFooter className="flex-col gap-2 text-sm ">
          <div className="leading-none text-muted-foreground">Displaying percentage distribution of payment method </div>
        </CardFooter>
      </Card>
      ...
      <CardHeader >
        ...
        <CardFooter className="flex-col gap-2 text-sm ">
          <div className="leading-none text-muted-foreground">
            Hover over the bar to see the product details
             </div>
        </CardFooter>
      </Card>
    </div>
